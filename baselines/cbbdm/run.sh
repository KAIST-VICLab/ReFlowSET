#!/usr/bin/env bash
# cBBDM (conditional BBDM, cBBDM-f4) SAR->EO.   GPU=0 bash run.sh <qxs|s2o>
#   job1: training
...
#      flip augmentation is ON for s2o and OFF for qxs -- see README.
#
...
#
# Environment
#   BASELINES_ROOT  vendored upstream repositories ($BASELINES_ROOT/cBBDM)
...
case "$DS" in
  qxs) CFG=configs/QXS-cBBDM-f4.yaml; TAG=QXS ;;
  s2o) CFG=configs/S2O-cBBDM-f4.yaml; TAG=S2O ;;
  *) echo "unknown dataset $DS"; exit 1 ;;
esac
PY=${PY:-python}
GPU=${GPU:-0}
# cBBDM's main.py only re-exports CUDA_VISIBLE_DEVICES for multi-GPU --gpu_ids;
# with a single id it uses cuda:<id> INSIDE the visible set, so --gpu_ids 0
# plus this export lands on physical GPU $GPU.
export CUDA_VISIBLE_DEVICES=$GPU

REPO=$BASELINES_ROOT/cBBDM
RES=$WORK_DIR/results/${DS}_cbbdm
LOGDIR=$WORK_DIR/logs
mkdir -p "$RES" "$LOGDIR"
cd "$REPO" || exit 1

overall=0
note() { echo "[$DS-cbbdm] $(date '+%F %T') $*"; }

# ---------------- Job 1: training ----------------
note "job1: cBBDM cBBDM-f4 train (GPU $GPU) -> $RES"
"$PY" main.py -c "$CFG" -t --gpu_ids 0 -r "$RES" \
    > "$LOGDIR/${DS}_cbbdm_train.log" 2>&1
rc=$?
note "job1 exit code $rc"

# ---------------- Job 2: sample_to_eval on the test set ----------------
CKPT="$RES/$TAG/cBBDM-f4/checkpoint/last_model.pth"
if [ $rc -eq 0 ] && [ -f "$CKPT" ]; then
    note "job2: sample_to_eval -> $RES/$TAG/cBBDM-f4/sample_to_eval/"
    "$PY" main.py -c "$CFG" --gpu_ids 0 -r "$RES" \
        --sample_to_eval --resume_model "$CKPT" \
        > "$LOGDIR/${DS}_cbbdm_sample.log" 2>&1
    rc=$?
